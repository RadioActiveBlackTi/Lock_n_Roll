...
if __name__=="__main__":
    ...
    x, _ = data_utils.dataset_extract(data, 1)
    y, _ = data_utils.dataset_extract(data, 8)
    z, _ = data_utils.dataset_extract(data, 11)
 ...
 [12.2   6.    7.06 10.2   8.86  1.06 -0.23 62.7 ]]"""
    ...
    net.eval()
    print(x, y, z)
    print(torch.FloatTensor(y).unsqueeze(0).shape)

    print('\nposition change test')
    print(net(torch.FloatTensor(x).unsqueeze(0), torch.FloatTensor(y).unsqueeze(0)))
    print(net(torch.FloatTensor(y).unsqueeze(0), torch.FloatTensor(x).unsqueeze(0)))

    print('\ndifferent test')
    print(net(torch.FloatTensor(z).unsqueeze(0), torch.FloatTensor(y).unsqueeze(0)))
    print(net(torch.FloatTensor(x).unsqueeze(0), torch.FloatTensor(z).unsqueeze(0)))

    print('\nsame test')
    print(net(torch.FloatTensor(x).unsqueeze(0), torch.FloatTensor(x).unsqueeze(0)))
    print(net(torch.FloatTensor(y).unsqueeze(0), torch.FloatTensor(y).unsqueeze(0)))

    print('\nlinear transformed data test')
    print(net(2*torch.FloatTensor(x).unsqueeze(0) + 6, 2*torch.FloatTensor(x).unsqueeze(0)))
    print(net(15*torch.FloatTensor(y).unsqueeze(0), 5*torch.FloatTensor(x).unsqueeze(0)+3))

    print('\nreal same data test')
    print(net(torch.FloatTensor(d).unsqueeze(0), torch.FloatTensor(t).unsqueeze(0)))
    print(net(torch.FloatTensor(t).unsqueeze(0), torch.FloatTensor(d).unsqueeze(0)))

    print('\nreal different data test')
    print(net(torch.FloatTensor(d).unsqueeze(0), torch.FloatTensor(x).unsqueeze(0)))
    print(net(torch.FloatTensor(y).unsqueeze(0), torch.FloatTensor(t).unsqueeze(0)))
    print(net(torch.FloatTensor(d).unsqueeze(0), torch.FloatTensor(z).unsqueeze(0)))
    print(net(torch.FloatTensor(y).unsqueeze(0), torch.FloatTensor(d).unsqueeze(0)))